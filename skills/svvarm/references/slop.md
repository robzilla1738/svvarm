...
svvarm's slop playbook — detecting generic, derivative, template-driven, and AI-influenced design patterns across visuals, copy, and interaction design. The full 38-pattern Anti-Slop Bible follows the audit methodology below.

The job is not to be theatrical. The job is to identify where the work feels mass-produced, unconsidered, or statistically assembled rather than intentionally designed.

Do not guess authorship from vibes alone. Audit patterns, evidence, repetition, and compound sameness. For positive references of what distinctive work looks like, pull `inspiration.md`.
...
If project context is missing, say so briefly and proceed with the code evidence you have. Do not invent brand intent.
...
A strong audit separates genuinely intentional work from statistically familiar output without becoming lazy, smug, or reckless. Anything less is incomplete.
...
Score using the Scoring Model defined at the top of this file (+4 definite, +2 borderline, +5 category penalty at 3+ patterns per category, +10 compound penalty at 4+ categories, +10 system penalty, optional intent credit). In practice, scores above 100 all mean the same thing: start over.
